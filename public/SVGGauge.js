let SVG = require('/lib/svg.js/dist/svg.min.js');
...
class RadialGauge {
    ...
    DrawNeedle(needleOptions) {
        ...
        return this.draw.line(
            ...
            this.options.radius + this.options.radius * (needleOptions.extension || 0), // y1
            this.options.radius, // x2
            this.options.radius * (needleOptions.gap || 0) // y2
    ...
    DrawTick(val, tickOptions, log) {
        ...
        gp.line(
            this.options.radius, // x1
            this.options.radius * tickOptions.outerGap, // y1
            this.options.radius, // x2
            this.options.radius * tickOptions.innerExtent // y2
        ).stroke(tickOptions.stroke) .id(`tickLine${val}`);
        if (tickOptions.label) gp.plain(val).attr({ x: this.options.radius, y: this.options.radius * tickOptions.label.extent })
